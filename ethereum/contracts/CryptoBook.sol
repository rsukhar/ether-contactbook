...
contract ContactFactory {
    ...
    modifier doesNotExist(){
        require(ownerToContact[msg.sender] == address(0), "You already created contact");
        _;
    }

    function createContact(string memory _telegram, string memory _discord) public doesNotExist {
        Contact contact = new Contact(msg.sender, _telegram, _discord);
        ownerToContact[msg.sender] = address(contact);
    }

    function createContact(string memory _telegram) public doesNotExist {
